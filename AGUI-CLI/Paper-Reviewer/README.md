...
The project has three main pieces: the agent, the knowledge base it uses, and the UI it talks through.
...
│       └── Dockerfile
├── knowledge_base/             # Same files, at project root for visibility
...
This tripped me up a few times, so here's a clear map.

**Agent config and environment variables** are in `agentcore/agentcore.json`. This is where the Knowledge Base ID, Data Source ID, and S3 bucket name are set. If you need to change any of those, that's the file.

**The knowledge base files** (agent_scope.md, analysis_schema.md, topic_dictionary.md, and the example templates) exist in two places: `knowledge_base/` at the project root so reviewers can actually find them, and `app/AnalyzerAgent/knowledge_base/` so they get copied into the Docker container at build time. They need to match.
...
**The CopilotKit UI** connects to the deployed agent via the runtime ARN in `copilotkit-ui/.env.local`. If you redeploy and get a new ARN, update it there.
...
The agent has access to a few custom tools beyond the standard Strands toolkit:
...
To understand what the agent does, start with `knowledge_base/agent_scope.md`. To see what it produces, look at the example files (prefixed with `_example_`).
